...
`CampaignPresetEditorDialog` creates either catalog type with a name, section, theme, and guidance body. Custom rows and their favorite IDs are stored in DataStore; decoders accept both the new five-field representation and the legacy three-field setting-template representation.

Active Adventure scenes obtain the bound persona ID from `RoleplayChatUiState` and combine that player with the immediate `PartyViewModel` cast. `CampaignCharacterCardsDialog` is available from both the scene header and the Other RPG action menu. It renders mode-aware portrait cards and routes each card to the existing full persona or character sheet. `RpgCharacterSheet` owns both d20 values and the RPG-only tactical profile (`tacticalRole`, ATK/DEF/SUP/SPD, AP/EP, and signature card fields). `createRpgCharacterSheet()` deterministically initializes complete class-aware values for manual, AI-created, persona-linked, daily-generated, and imported characters; serialization defaults keep older sheets compatible.
