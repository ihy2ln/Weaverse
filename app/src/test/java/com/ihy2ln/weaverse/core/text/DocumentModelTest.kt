...
class DocumentModelTest {
    ...
    fun jsonRoundTrip_preservesMediaPlacementAmongProse() {
        ...
        assertEquals(
            listOf("media-abc", "media-a", "media-b", "media-c", "media-d"),
    ...
    @Test
    fun readableText_skipsMediaAndSceneBeats() {
        val doc = Document(
            listOf(
                Paragraph("p1", listOf(Span("Prose"))),
                MediaBlock("m1", "img-a", MediaKind.Image),
                Heading("h1", 2, listOf(Span("Chapter"))),
            ),
        )
        assertEquals("Prose", doc.blocks[0].readableText())
        assertEquals("", doc.blocks[1].readableText())
        assertEquals("Chapter", doc.blocks[2].readableText())
    }
}
